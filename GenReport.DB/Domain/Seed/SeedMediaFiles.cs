...
namespace GenReport.DB.Domain.Seed
{
    public partial class ApplicationDBContextSeeder
    {
        ...
        {
            var files = Enumerable.Range(0, 20).Select(x => new MediaFile(storageURL: Faker.Internet.SecureUrl(), fileName: $"{Faker.Name.FullName()}-profilepic.png",mimeType:  MimeTypes[Faker.RandomNumber.Next(MimeTypes.Count-1)],size:Faker.RandomNumber.Next(short.MaxValue)));
